...
// recatStore seeds the same two-expense fixture the recategorize test relies on.
...
func TestRecategorize(t *testing.T) {
	...
	// Category breakdown now lumps both under Shopping; totals are unchanged.
	totalExp := 0
	for _, c := range s.CategoryBreakdown(serialOf(2026, 6, 1), serialOf(2026, 6, 30), Expense) {
		totalExp += c.Total
	}
	if totalExp != 149000 {
		t.Fatalf("expenses total changed to %d, want 149000", totalExp)
	}
